...
function processTreeViewItems(data, operations) {
    ...
    result = handleOperation(result, consts_1.EXPAND_FIELD, operations.expand, cloneField);
    result = handleOperation(result, consts_1.SELECT_FIELD, operations.select, cloneField);
    result = handleOperation(result, consts_1.CHECK_FIELD, operations.check, cloneField);
...
function handleOperation(items, defaultField, operation, cloneField) {
    if (operation) {
        ...
        var indices = !misc_1.isArray(operation) && operation.idField ? itemUtils_1.resolveItemsIds(ids, operation.idField, items) : ids;
...
function raiseFlags(items, indices, field, cloneField) {
    ...
    indices.forEach(function (itemIndex) {
        result = itemUtils_1.updateItem(result, itemIndex, function (item) { return raiseNestedFlag(field, item); }, cloneField);
    });
    return result;
}
